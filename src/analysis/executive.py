...
def get_executive_summary():
    ...
    # 4. Finansal etki — her makine için kendi pph ile hesapla (P11 spike'ları capped)
    # Counter spike sorunu olan makineler (P11) için pph aşırı yüksek çıkıyor.
    # Bu yüzden makine bazlı çağırıyoruz ve maks 50 parça/saat cap uyguluyoruz.
    annual_benefit = 0
    daily_per_machine_total = 0
    valid_machine_count = 0
    for md in machines_data:
        if md["improvement_pp"] <= 0:
            continue
        try:
            fi = calculate_financial_impact(md["improvement_pp"], md["machine"])
            # Spike'lı makineleri tespit et: günlük benefit aşırı yüksekse normalize et
            daily = fi["net_benefit_per_day"]
            if daily > 5000:  # Şüpheli — counter spike etkisi
                daily = min(daily, 2500)  # Mantıklı tavan
            annual_benefit += daily * 365
            daily_per_machine_total += daily
            valid_machine_count += 1
        except Exception:
            pass

    # Backwards-compat için aggregate financial obj de oluştur
    financial = None
    if valid_machine_count > 0:
        avg_daily = daily_per_machine_total / valid_machine_count
        try:
            template = calculate_financial_impact(avg_improvement)
            payback = round(300 / max(avg_daily, 0.01), 2)
            financial = {
                "delta_oee": round(avg_improvement, 4),
                "recovered_hours_per_day": round(avg_improvement * 24, 2),
                "extra_pieces_per_day": template.get("extra_pieces_per_day", 0),
                "gross_benefit_per_day": template.get("gross_benefit_per_day", 0),
                "downtime_saving_per_day": template.get("downtime_saving_per_day", 0),
                "net_benefit_per_day": round(avg_daily, 2),
...
def compare_machines(machines: list = None):
    ...
    if not machines:
        machines = ["Makine 1", "Makine 2", "Makine 3", "Makine 5", "Makine 7", "Makine 9"]
